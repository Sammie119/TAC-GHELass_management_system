...
@section('page-title', 'Archived Income')
@section('content')

    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;flex-wrap:wrap;gap:12px;">
        <div>
            <h2 style="font-size:18px;font-weight:600;color:#111827;">Archived Income Records</h2>
            ...
            @forelse($records as $record)
                <tr style="border-top:1px solid #f3f4f6;opacity:0.8;"
                    ...
                    <td style="padding:12px 16px;">
                        <p style="font-weight:500;color:#6b7280;">{{ $record->member?->full_name ?? 'Anonymous' }}</p>
                    ...
                    <td style="padding:12px 16px;">
                        <div style="display:flex;gap:10px;align-items:center;">
                            {{-- Restore --}}
                            <form method="POST" action="{{ route('admin.finance.income.restore', $record->id) }}">
                                @csrf
                                <button type="submit"
                            ...
                            @role('admin')
                                <form method="POST"
                                      action="{{ route('admin.finance.income.force-delete', $record->id) }}"
